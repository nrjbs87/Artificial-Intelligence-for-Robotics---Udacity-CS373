...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        steerAngle = motion[0] 
        D = motion[1]
        beta = (D/self.length) * tan(steerAngle)
        if beta < 0.01:
            ...
            newX = Cx + sin(self.orientation + beta) * R
            newY = Cy - cos(self.orientation + beta) * R
         
        
        newOrientation = (self.orientation + beta) % (2*pi)

        newX += random.gauss(0.0, self.distance_noise)
        newY += random.gauss(0.0, self.distance_noise)
        newOrientation += random.gauss(0.0, self.steering_noise)
    ...
    def sense(self, add_noise = 1): #do not change the name of this function
        ...
        for LM in landmarks:
            ...
            LMBearing = (atan2(dy, dx) - self.orientation) % (2*pi)

            if add_noise:
                LMBearing += random.gauss(0.0, self.bearing_noise)
